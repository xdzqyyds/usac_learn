 ...
 *	
 *	Enhanced Multi-Pulse Excitation Encoding Subroutines
 *
		...
		long	*ga_idx )	/* output */
{
   ...
   if((pul_amp = (float *)calloc (cand_gain*num_pulse[num_enh], sizeof(float)))==NULL) {
      printf("\n Memory allocation error in nec_enh_mp_enc \n");
      exit(1);
   }
   if((pul_loc = (long *)calloc (cand_gain*num_pulse[num_enh], sizeof(long)))==NULL) {
      printf("\n Memory allocation error in nec_enh_mp_enc \n");
      exit(1);
   }
   for ( i = 0; i < num_prune; i++ ) {
      if((loc[i] = (long *)calloc (num_pulse[num_enh], sizeof(long)))==NULL) {
	 printf("\n Memory allocation error in nec_enh_mp_enc \n");
	 exit(1);
      }
      if((loc2[i] = (long *)calloc (num_pulse[num_enh], sizeof(long)))==NULL) {
	 printf("\n Memory allocation error in nec_enh_mp_enc \n");
	 exit(1);
      }
   }
   ...
   for(l = 0; l < cand_gain; l++){
	 ...
	 pul_loc[l*num_pulse[num_enh]+i] = loc[l][i];
	 pul_amp[l*num_pulse[num_enh]+i] = sgn[loc[l][i]];
      }
   }
   ...
   nec_enh_gain_enc(vu_flag,pul_loc,pul_amp,num_pulse[num_enh],I_part,
		    cand_gain,g_ac,g_ec,qxnorm,z,ac,syn_ac,alpha,g_den,g_num,
   ...
   for (i = 0; i < num_pulse[num_enh]; i++) tmp_exc[pul_loc[i]] = pul_amp[i];
