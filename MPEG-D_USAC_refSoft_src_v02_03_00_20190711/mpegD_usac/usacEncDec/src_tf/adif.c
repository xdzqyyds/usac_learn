...
{
  if ( (profile < 0) || (profile > LTP_Profile))
    return 1;
  else
    return 0;
}
...
static void GetProgConfig( HANDLE_BSBITSTREAM  bitstream, ADIF_INFO* adifInfo )
{
...
int GetAdifHeader( HANDLE_BSBITSTREAM  bitstream, ADIF_INFO* adifInfo )
{

  int i;
  int ADIF_present = 0;
  unsigned long data;
  HANDLE_BSBITBUFFER  bitBuffer;
  ...
  if ( !strncmp( (char*)BsBufferGetDataBegin(bitBuffer), ADIF_ID, strlen(ADIF_ID))) {
