...
void ntt_scale_dec_bark_env ( int    nfr,
                              int    nsf,
                              int    n_ch,
                              double *codebook,
                              int    ndiv,
                              ...
                              double band_lower[])
{
  ...
  for (i_ch=0; i_ch<n_ch; i_ch++){
    ...
    for (isf=0; isf<nsf; isf++){
      subtop = isf * nfr + top;
      for(ismp=0; ismp<nfr; ismp++) bark_env[ismp+subtop] = 1.0;
      idtop  = i_ch * nsf + isf;
      /* Initialization 
      ...
      */ 
      ...
      for (ib=0, ismp = (int) ( nfr*band_lower[i_ch] ) /*0*/; ib<n_crb; ib++){
	dtmp = ntt_max(env[ib]+alfq*penv[ib]+1., ntt_FW_LLIM);
      for (; ismp<bark_tbl[i_ch*n_crb+ib]; ismp++){
