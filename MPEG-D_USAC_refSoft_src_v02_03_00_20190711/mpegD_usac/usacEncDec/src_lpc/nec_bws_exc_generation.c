...
)
{
   ...
   pitch_max = NEC_PITCH_MAX_FRQ16;
      ...
      if (flag_mem == 0 || ((c_subframe % n_subframes) == 0 && prev_vad != 1 )){
...
#ifdef VERSION2_EC	/* SC: 1, EC: RMS, BWS */
   /* error concealment of RMS */
   {
	...
	if( act_flag & EA_RMS_PREVIOUS ){
		...
		if( act_flag & EA_RMS_REDUCE ){
			...
			last_rms *= pow( 10.0, ((float)-3276/8192)/20.0 );	/* -0.4dB */
		    else
			last_rms *= pow( 10.0, ((float)-9830/8192)/20.0 );	/* -1.2dB */
		}
		qxnorm[c_subframe] = last_rms;
	}
	errConSaveRmsBws( qxnorm[c_subframe] );
   }
#endif
