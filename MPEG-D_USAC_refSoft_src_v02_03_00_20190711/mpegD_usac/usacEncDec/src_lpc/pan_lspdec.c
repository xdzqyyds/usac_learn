...
/* 2nd stage quantized value */
	...
	if(idx[4]==0) {
		...
		if(flagPred != 0) {
		    ...
		    if((int)idx[2]<ncd_2[0]) {
		        for(i=0;i<dim_2[0];i++) {
			    out[i] = (float)((p_factor*out_p[i]+(1.-p_factor)*out[i])
		    ...
		    if((int)idx[3]<ncd_2[1]) {
		        for(i=0;i<dim_2[1];i++) {
			    out[dim_2[0]+i] = (float)((p_factor*out_p[dim_2[0]+i]
...
void pan_lspdecEP(
    ...
    long flagStab, /* in: 0 - stabilization OFF, 1 - stabilization ON */
    long flagPred, /* in: 0 - LSP prediction OFF, 1 - LSP prediction ON */
    long sameVUV, /* in: 1 - same V/UV flag, 0 - different V/UV flag */
    long crcResultEP[], /* in: crc results */
    long crcErrCnt /* in: error count */
    )
{
	long	i;
	float	dif, mindif, x;
...
/* 2nd stage quantized value */
	...
	if(idx[4]==0) {
		...
		if(flagPred != 0) {
                    if ( crcResultEP[1] ) {
			for (i=0; i<dim_1[0]+dim_1[1]; i++)
			    out[i] = out_p[i];
                    } else if ( crcResultEP[0] ) {
			if ( sameVUV ) {
			    p_factor = (float)(p_factor - 0.1 * (float)crcErrCnt);
			    if (p_factor < 0.0) p_factor = 0.0;
		            for(i=0;i<dim_1[0]+dim_1[1];i++) 
			        out[i] = (float)(p_factor*out_p[i]+(1.-p_factor)*out[i]);
                        } else {
			    mindif = 1.0;
			    for (i = 0; i < lpc_order-1; i++) {
			        if ((dif = (float)fabs(out[i+1] - out[i])) < mindif)
				     mindif = dif;
			    }
			    if (mindif < 0.025) {
			        x = (float)((1./(float)(lpc_order+1) - 0.025)
			          / (1./(float)(lpc_order+1) - mindif));
			        for (i = 0; i < lpc_order; i++)
			            out[i] = (float)(x * out[i] + (1.-x)
			                * (float)(i+1)/(float)(lpc_order+1));
			    }
			}
	            } else {
		        if((int)idx[2]<ncd_2[0]) {
