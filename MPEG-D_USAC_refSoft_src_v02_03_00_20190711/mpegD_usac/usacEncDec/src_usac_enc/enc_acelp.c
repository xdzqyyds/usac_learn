...
$Id: enc_acelp.c,v 1.2 2010-03-12 06:54:42 mul Exp $
*******************************************************************************/
...
void E_ACELP_4t(Float32 dn[], Float32 cn[], Float32 H[], Word16 code[],
                Float32 y[], Word32 nbbits, Word16 mode, int _index[])
{
   ...
   for (k = 0; k < nbiter; k++)
   {
...
#else
      if (nbbits == 20 | (nbbits == 12) | (nbbits == 16))
#endif
      {
         pos = 0;
         ps = 0.0F;
      ...
      {
		if((nb_pulse-j)>=2){ /*pair-wise search*/
         /*
  ...
  if (nbbits == 12){
    for (track = 0; track < 4; track++) {
      ...
      if(ind[k]>=0){
        ...
        _index[2*(track%2)+1] = E_ACELP_quant_1p_N1(ind[k], 4);
      }
    }
  } 
  else if (nbbits == 16) {
    ...
    for (track = 0; track < 4; track++)
      {
        k = track * NPMAXPT;
        if(ind[k]>=0){
          _index[i++] = E_ACELP_quant_1p_N1(ind[k], 4);
        }else{
          if(track==1) {
            _index[0] = 0;
          } else if( track==3) {
