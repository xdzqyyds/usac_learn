                         ...
                         int *ppredCoefQInt)
{
  ...
  for (imdct = 0; imdct < nmdct; imdct++) {
    emid += (float)(mdctMid[imdct]*mdctMid[imdct]);
    cov  += (float)(mdctMid[imdct]*mdctSide[imdct]);
  }

  *ppredCoef = cov/(emid + eps);

  signPredCoef = *ppredCoef > 0 ? 1 : -1;
  absPredCoef = min((float)fabs(*ppredCoef), k_max);
                        ...
                        double *mdctSide)
{
  ...
  for (imdct = 0; imdct < nmdct; imdct++) {
    mdctRes = (float)(mdctSide[imdct] - predCoefQ*mdctMid[imdct]);
