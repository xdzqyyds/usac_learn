...
首先需要明确，QT中的音频采集模式和读取文件模式是否有差别

测试在低码率下，看看是否有问题


最低采样率`min_sample_rate = 12000HZ`
最低码率支持情况计算公式
```
单通道：min_birate = sample_rate/3 + 1600
双通道: min_birate = sample_rate/3 + 3100
```
经检验，可以实现最低码率
但是目前解码端并没有对应的采样率的初始化文件，因此需要添加对应的采样率文件，再重新进行封装等。
目前已经实现采样率：
```
48000HZ 44100HZ 32000HZ
期望添加如下采样率 12000HZ 16000HZ 24000HZ
```

首先通过ffmepg生成正弦波，接着将其作为输入序列，通过编码器将指定编码只包含一帧的.mp4头文件