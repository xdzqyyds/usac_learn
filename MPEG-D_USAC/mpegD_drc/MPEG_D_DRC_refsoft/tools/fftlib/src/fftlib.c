...
static int CFFTN_NI(float *InRealData,
                    float *InImagData,
                    float *OutRealData,
                    float *OutImagData,
                    int len, int isign);
...
int FFTlib_Create(HANDLE_FFTLIB* phFFT, int len, int iSign)
{
  HANDLE_FFTLIB hFFT;
  ...
  *phFFT = 0;
  ...
  if( iSign != 1 && iSign != -1 ){
    return -1;   /* neither forward nor backward FFT*/
  }
  
  if( 0 == (hFFT = (HANDLE_FFTLIB)calloc(1, sizeof(FFTLIB)))){
    return -1;
  }
  
  for( hFFT->shift = 1; len>>hFFT->shift != 2 && len>>hFFT->shift != 3 && len>>hFFT->shift != 5 && len>>hFFT->shift != 0;  hFFT->shift++ );
  hFFT->len   = len;
  hFFT->iSign = iSign;
...
int FFTlib_Apply(HANDLE_FFTLIB hFFT, float* pInReBuffer, float* pInImBuffer, float* pOutReBuffer, float* pOutImBuffer)
{
  if( hFFT != 0){
    if (0 != !(CFFTN_NI(pInReBuffer, pInImBuffer, pOutReBuffer, pOutImBuffer, hFFT->len, hFFT->iSign))){
      return -1;
    }
...
void FFTlib_Destroy(HANDLE_FFTLIB* phFFT)
{
