                    ...
                    UniDrcConfigExt* uniDrcConfigExt)
{
    ...
    if (drcCoeffsAndInstructionsUniDrcV1Present == 1)
    {
        ...
        uniDrcConfig->drcInstructionsUniDrcV1Count = drcInstructionsUniDrcV1Count;
    }
...
static int
effectIsDuckingSelf(int drcSetEffect)
{
    return !!(drcSetEffect & (1 << 11));
}

static int
parseLoudnessLevelingConfigExtension(
    ...
    DrcParamsBsDec *drcParams)
{
...
#if AMD1_SYNTAX == 1
    int const drcInstructionsUniDrcCount = uniDrcConfig->drcInstructionsUniDrcV1Count;
    int const version = 1;
#elif MPEG_H_SYNTAX == 1
    ...
    {
        if(!effectIsDuckingSelf(uniDrcConfig->drcInstructionsUniDrc[i].drcSetEffect)) continue;

        err = getBits(bitstream, 1, &uniDrcConfig->drcInstructionsUniDrc[i].levelingPresent);
        if(err) return err;

        if(uniDrcConfig->drcInstructionsUniDrc[i].levelingPresent)
        {
            int duckingOnlyDrcSetPresent = 0;

            err = getBits(bitstream, 1, &duckingOnlyDrcSetPresent);
            if(err) return err;

            if(duckingOnlyDrcSetPresent)
            {
                ++uniDrcConfig->drcInstructionsUniDrcCount;
                assert(uniDrcConfig->drcInstructionsUniDrcCount <= DRC_INSTRUCTIONS_COUNT_MAX);

                DrcInstructionsUniDrc *drcInstructionsUniDrc =
                    &uniDrcConfig->drcInstructionsUniDrc[uniDrcConfig->drcInstructionsUniDrcCount-1];

                drcInstructionsUniDrc->duckingOnlyDrcSet = 1;

 #if MPEG_H_SYNTAX == 1
                drcInstructionsUniDrc->drcInstructionsType = uniDrcConfig->drcInstructionsUniDrc[i].drcInstructionsType;
                if (uniDrcConfig->drcInstructionsUniDrc[i].drcInstructionsType == 2)
                    drcInstructionsUniDrc->mae_groupID = uniDrcConfig->drcInstructionsUniDrc[i].mae_groupID;
                else if (uniDrcConfig->drcInstructionsUniDrc[i].drcInstructionsType == 3)
                    drcInstructionsUniDrc->mae_groupPresetID = uniDrcConfig->drcInstructionsUniDrc[i].mae_groupPresetID;
 #endif

                err = parseDrcInstructionsUniDrc(
                    bitstream,
                    version,
                    uniDrcConfig,
                    &uniDrcConfig->channelLayout,
                    drcParams,
    ...
    while(uniDrcConfigExt->uniDrcConfigExtType[k] != UNIDRCCONFEXT_TERM)
    {
        ...
        switch(uniDrcConfigExt->uniDrcConfigExtType[k])
        {
            ...
            case UNIDRCCONFEXT_LEVELING:
                err = parseLoudnessLevelingConfigExtension(bitstream, uniDrcConfig, drcParams);
                if(err) return err;
                break;
#endif
            /* add future extensions here */
