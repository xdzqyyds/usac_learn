...
extern const char usac_binaries[][3][FILENAME_MAX];
...
encode_obj* xheaace_create(encode_para* encode_para) {
    ...
    encPara->bitrate = encode_para->bitrate;
    ...
    if (encode_para->input_file_flag == 1) {
        audioFileName = wav_InputFile;
        bitFileName = mp4_OutputFile;
    } else {
        audioFileName = strdup("noinput.wav");
        bitFileName = mp4_OutputFile;
    }
    ...
    if (encode_para->input_file_flag == 1) {
        ...
        audioFile->numChannel = encode_para->ui_num_chan;
        numChannel = encode_para->ui_num_chan;
        fSample = encode_para->ui_samp_freq;
        fileNumSample = 40960;
    ...
    ctx->sbrenable = sbrenable;
    ctx->numChannel = numChannel;
    ctx->frameNumSample = frameNumSample;
    ctx->downSampleRatio = downSampleRatio;
    ctx->upSampleRatio = upSampleRatio;
    ctx->delayTotal = delayTotal;
    ctx->NcoefRS = NcoefRS;
    ctx->NcoefUS = NcoefUS;
    ctx->delayBufferSamples = delayBufferSamples;
    ctx->addlCoreDelay = addlCoreDelay;
    ctx->delayEncFrames = delayEncFrames;
    ctx->numAPRframes = numAPRframes;
    ctx->frame = frame;
    ctx->requestIPF = requestIPF;
    ctx->enableIPF = enableIPF;
    ctx->numChannelBS = numChannelBS;
    ctx->track_count_total = track_count_total;
    ctx->err = err;
    ctx->nSamplesProcessed = nSamplesProcessed;

    ctx->hRS = hRS;
    ctx->hUS = hUS;

    ctx->sampleBuf = sampleBuf;
    ctx->sampleBufRS = sampleBufRS;
    ctx->sampleBufRSold = sampleBufRSold;
    ctx->sampleBufUS = sampleBufUS;
    ctx->sampleBufUSold = sampleBufUSold;
    ctx->additionalCoreDelayBuffer = additionalCoreDelayBuffer;
    ctx->tmpBufRS = tmpBufRS;
    ctx->tmpBufUS = tmpBufUS;
    ctx->encSampleBuf = encSampleBuf;
    ctx->reSampleBuf = reSampleBuf;

    ctx->frameData = frameData;
    ctx->au_buffers = au_buffers;
    ctx->outprog = outprog;

    ctx->audioFile = audioFile;
    ctx->outfile = outfile;
    ctx->fSampleLong = fSampleLong;
...
int xheaace_encode_frame(encode_obj* ctx, const unsigned char* raw_pcm_frame, unsigned char** out_encoded_data, int* out_encoded_size) {
    ...
    *out_encoded_size = 0;
    ...
    if (ctx->sbrenable) {
        /* save last NcoefRS samples */
        for (ch = 0; ch < ctx->numChannel; ch++)
            for (i = 0; i < ctx->NcoefRS + ctx->delayTotal; i++)
                ctx->sampleBufRSold[ch][i] = ctx->sampleBufRS[ch][ctx->downSampleRatio * ctx->frameNumSample - (ctx->NcoefRS + ctx->delayTotal) + i];

        for (ch = 0; ch < ctx->numChannel; ch++)
            for (i = 0; i < ctx->NcoefUS / ctx->upSampleRatio; i++)
                ctx->sampleBufUSold[ch][i] = ctx->sampleBufUS[ch][ctx->downSampleRatio * ctx->frameNumSample / ctx->upSampleRatio - (ctx->NcoefUS / ctx->upSampleRatio) + i];
    }
    ...
    /* 20060107 */
    if (ctx->sbrenable) {
        if (ctx->input_file_flag == 1) {
            numSample = AudioReadData(ctx->audioFile, ctx->sampleBufRS, ctx->frameNumSample * ctx->downSampleRatio / ctx->upSampleRatio);
        } else {
            numSample = ctx->frameNumSample * ctx->downSampleRatio / ctx->upSampleRatio;
            convert_raw_pcm_to_float_simple(raw_pcm_frame, buf, numSample * ctx->numChannel);
            for (i = 0; i < numSample * ctx->numChannel; i++){
                ctx->sampleBufRS[(i) % ctx->numChannel][(i) / ctx->numChannel] = buf[i];
            }
        }

        raw_data = ctx->sampleBufRS[0];

        /* Consider an additional delay caused by the core encoder */
        for (ch = 0; ch < ctx->numChannel; ch++) {
            coreDelay = ctx->delayBufferSamples;
            nSamples = ctx->frameNumSample * ctx->downSampleRatio / ctx->upSampleRatio;
    
            for (i = 0; i < coreDelay; i++) {
                ctx->additionalCoreDelayBuffer[ch][i] = ctx->sampleBufRS[ch][nSamples - coreDelay + i];
            }
            for (i = nSamples - 1; i >= coreDelay; i--) {
                ctx->sampleBufRS[ch][i] = ctx->sampleBufRS[ch][i - coreDelay];
            }
            for (i = 0; i < coreDelay; i++) {
                ctx->sampleBufRS[ch][i] = ctx->additionalCoreDelayBuffer[ch][2 * nSamples - coreDelay + i];
                ctx->additionalCoreDelayBuffer[ch][2 * nSamples - coreDelay + i] = ctx->additionalCoreDelayBuffer[ch][i];
            }
        }
    }
    /* read audio file */
    ...
    if (ctx->sbrenable) {
        ...
        if (ctx->upSampleRatio > 1) {
            for (i = 0; i < ctx->downSampleRatio * ctx->frameNumSample / ctx->upSampleRatio; i++)
                for (ch = 0; ch < nChannelsResamp; ch++)
                    ctx->sampleBufUS[ch][i] = ctx->sampleBufRS[ch][i];

            for (i = 0; i < ctx->NcoefUS / ctx->upSampleRatio; i++)
                for (ch = 0; ch < nChannelsResamp; ch++)
                    ctx->tmpBufUS[ch][i] = ctx->sampleBufUSold[ch][i];

            for (i = ctx->NcoefUS / ctx->upSampleRatio; i < ctx->downSampleRatio * ctx->frameNumSample / ctx->upSampleRatio + (ctx->NcoefUS / ctx->upSampleRatio); i++)
                for (ch = 0; ch < nChannelsResamp; ch++)
                    ctx->tmpBufUS[ch][i] = ctx->sampleBufUS[ch][i - (ctx->NcoefUS / ctx->upSampleRatio)];

            for (ch = 0; ch < nChannelsResamp; ch++)
                FIupsample(ctx->tmpBufUS[ch], ctx->sampleBufRS[ch], ctx->downSampleRatio * ctx->frameNumSample, ctx->hUS, ctx->NcoefUS, ctx->upSampleRatio);

        }

        for (i = 0; i < (ctx->NcoefRS + ctx->delayTotal); i++)
            for (ch = 0; ch < nChannelsResamp; ch++)
                ctx->tmpBufRS[ch][i] = ctx->sampleBufRSold[ch][i];

        /* Add NcoefRS samples delay to input signal sampleBufRS */
        for (i = (ctx->NcoefRS + ctx->delayTotal); i < ctx->downSampleRatio * ctx->frameNumSample + (ctx->NcoefRS + ctx->delayTotal); i++)
            for (ch = 0; ch < nChannelsResamp; ch++)
                ctx->tmpBufRS[ch][i] = ctx->sampleBufRS[ch][i - (ctx->NcoefRS + ctx->delayTotal)];

#ifdef USE_AFSP
        /* Low-pass filtering and downsampling */
        for (ch = 0; ch < nChannelsResamp; ch++)
            FIdownsample(ctx->tmpBufRS[ch] + ctx->addlCoreDelay, ctx->sampleBuf[ch], ctx->frameNumSample, ctx->hRS, ctx->NcoefRS, ctx->downSampleRatio);
#else
        CommonExit(1, "AFSP not available.");
#endif
    }

    if (ctx->delayEncFrames - ctx->numAPRframes == ctx->frame) {
        ctx->requestIPF = (1 == ctx->enableIPF) ? 1 : 0;
    }
    else {
        ctx->requestIPF = 0;
    }
    ...
    /* encode one frame */
    ctx->track_count_total = 0;
    
    /* mix down stereo to mono */
    ctx->encSampleBuf = ctx->sampleBuf;
    if ((ctx->numChannelBS == 1) && (ctx->numChannel == 2)) {
        for (i = 0; i < ctx->frameNumSample; i++) {
            ctx->reSampleBuf[0][i] = (ctx->sampleBuf[0][i] + ctx->sampleBuf[1][i]) / 2;
        }
        ctx->encSampleBuf = ctx->reSampleBuf;
    }

    ctx->err = ctx->frameData.enc_mode->EncFrame(ctx->encSampleBuf,
        &ctx->au_buffers[ctx->track_count_total],
        ctx->requestIPF,
        &usacSyncState,
        *ctx->frameData.enc_data,
        ctx->sbrenable ? ((ctx->upSampleRatio > 1) ? ctx->tmpBufUS : ctx->tmpBufRS) : NULL);

    if (0 != ctx->err) {
        CommonExit(1, "EncFrame returned %i", ctx->err);
    }

    ctx->track_count_total += *ctx->frameData.trackCount;

    if (ctx->frame >= ctx->delayEncFrames) {
        for (i = 0; i < ctx->track_count_total; i++) {
            HANDLE_STREAM_AU au = StreamFileAllocateAU(0);
            ...
            *out_encoded_size = byte_size;
            ...
            switch (usacSyncState) {
            case USAC_SYNCFRAME_IMMEDIATE_PLAY_OUT_FRAME:
                ctx->outprog->sampleGroupID[i] = SAMPLE_GROUP_NONE;
                ctx->outprog->isIPF[i] = 1;
                break;
            case USAC_SYNCFRAME_INDEPENDENT_FRAME:
                ctx->outprog->sampleGroupID[i] = SAMPLE_GROUP_PROL;
                ctx->outprog->isIPF[i] = 0;
                break;
            default:
                ctx->outprog->sampleGroupID[i] = SAMPLE_GROUP_NONE;
                ctx->outprog->isIPF[i] = 0;
            ...
            StreamPutAccessUnit(ctx->outprog, i, au);
            BsClearBuffer(ctx->au_buffers[i]);
            free(au);
        }
    }
...
int xheaace_delete(encode_obj* ctx, const char* encoded_file) {
    ...
    fprintf(stdout, "totNumFrame=%d\n", ctx->frame - ctx->delayEncFrames);

    /* free encoder memory */
    if (ctx->frameData.enc_data && ctx->frameData.enc_mode) {
        ctx->frameData.enc_mode->EncFree(*ctx->frameData.enc_data);
        if (ctx->frameData.enc_data) {
            free(ctx->frameData.enc_data);
        }
        free(ctx->frameData.enc_mode);
    }

    if (ctx->frameData.trackCount) {
        free(ctx->frameData.trackCount);
    }
    ...
    /* editlist support for USAC */
#ifdef EDITLIST_SUPPORT
        StreamFileSetEditlistValues(ctx->outprog,
            0,
            ctx->fSampleLong,
            1024 * (ctx->sbrenable + 1) * ctx->frame,
            ctx->fSampleLong,
            0,
            ctx->frame - ctx->delayEncFrames);
#endif
    ...
    /* close bit stream file */
    ctx->outfile->fileName = encoded_file;

    if (ctx->output_file_flag == 1) {
        if (StreamFileClose(ctx->outfile))
            CommonExit(1, "Encode: error closing bit stream file");
    }
    ...
    /* free buffers */
    for (ch = 0; ch < ctx->numChannel; ch++) {
        free(ctx->sampleBuf[ch]);
        free(ctx->reSampleBuf[ch]);
    }
    free(ctx->sampleBuf);
    free(ctx->reSampleBuf);
    if (ctx->au_buffers) {
        for (i = 0; i < ctx->track_count_total; i++) {
            BsFreeBuffer(ctx->au_buffers[i]);
        }
        free(ctx->au_buffers);
    }
    ...
    if (ctx->sampleBufRS) {
        for (ch = 0; ch < ctx->numChannel; ch++) {
            if (ctx->sampleBufRS[ch]) {
                free(ctx->sampleBufRS[ch]);
            }
        }
        free(ctx->sampleBufRS);
    }

    if (ctx->sampleBufUS) {
        for (ch = 0; ch < ctx->numChannel; ch++) {
            if (ctx->sampleBufUS[ch]) {
                free(ctx->sampleBufUS[ch]);
            }
        }
        free(ctx->sampleBufUS);
    }

    if (ctx->sampleBufRSold) {
        for (ch = 0; ch < ctx->numChannel; ch++) {
            if (ctx->sampleBufRSold[ch]) {
                free(ctx->sampleBufRSold[ch]);
            }
        }
        free(ctx->sampleBufRSold);
    }

    if (ctx->sampleBufUSold) {
        for (ch = 0; ch < ctx->numChannel; ch++) {
            if (ctx->sampleBufUSold[ch]) {
                free(ctx->sampleBufUSold[ch]);
            }
        }
        free(ctx->sampleBufUSold);
    }

    if (ctx->tmpBufRS) {
        for (ch = 0; ch < ctx->numChannel; ch++) {
            if (ctx->tmpBufRS[ch]) {
                free(ctx->tmpBufRS[ch]);
            }
        }
        free(ctx->tmpBufRS);
    }
