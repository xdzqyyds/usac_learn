...
)
{
   ...
   if(fs8kHz==SampleRateMode) {
     if ( lagbit != NEC_ACB_BIT ) {
       printf("\n Configuration error in nec_enc_acb NB\n");
       exit(1);
     }
   }else {
     if ( lagbit != NEC_ACB_BIT_WB ) {
       printf("\n Configuration error in nec_enc_acb WB\n");
       exit(1);
     }
   }
   ...
   if(fs8kHz==SampleRateMode) {
     if ((mem_ac =(float *)calloc(NEC_PITCH_MAX+NEC_PITCH_IFTAP+1+len_sf, sizeof(float))) == NULL) {
        printf("\n Memory allocation error in nec_enc_acb \n");
        exit(1);
     }
   }else {
     if ((mem_ac =(float *)calloc(NEC_PITCH_MAX_FRQ16+NEC_PITCH_IFTAP16+1+len_sf, sizeof(float))) == NULL) {
        printf("\n Memory allocation error in nec_enc_acb \n");
        exit(1);
     }
   }
   ...
   /*--- DELAY SEARCH ---*/
   if(fs8kHz==SampleRateMode) {
     for ( i = 0; i < NEC_PITCH_MAX + NEC_PITCH_IFTAP+1; i++)
        mem_ac[i] = mem_past_exc[i];
   }else {
     for ( i = 0; i < NEC_PITCH_MAX_FRQ16 + NEC_PITCH_IFTAP16+1; i++)
        mem_ac[i] = mem_past_exc[i];
   }
   for ( i = 0; i < len_sf; i++ ) zero[i] = 0.0;

   *ac_idx_opt = op_idx;
   *int_part = nec_acb_generation( *ac_idx_opt, len_sf, mem_ac,
				  zero, ac, 1.0, 0, SampleRateMode );
   ...
   for (idx = st_idx; idx <= ed_idx; idx++) {
      I_part = nec_acb_generation( idx, len_sf, mem_ac, zero, exc, 1.0, 0, 
                SampleRateMode );
