		  ...
		  long	*ga_idx )	/* output */
{
   ...
   */
   ...
   if((Eg = (float *)calloc (size_gc, sizeof(float)))==NULL) {
   ...
   codebook = (4 * n_cb) + vu_flag;      /*  RF 991123   */
   ...
   *g_ec = 0.0;
   ...
   for (k = 0; k < cand_gain; k++)
   {
       ...
       {
           /*  RF 991123   */ 
           offset = NEC_GAIN_DIM * jg;

           if (gainbit == NEC_BIT_GAIN)
           {
               nga = nec_gc [codebook][offset] * renorm * ivnorm1;
               ngc = nec_gc [codebook][offset + 1] * renorm * ivnorm2[k];
           }
           else
           {
               nga = nec_wb_gc [codebook][offset] * renorm * ivnorm1;
               ngc = nec_wb_gc [codebook][offset + 1] * renorm * ivnorm2[k];
           }

           /*
           nga = gcb[NEC_GAIN_DIM*jg+0] * renorm * ivnorm1;
           ngc = gcb[NEC_GAIN_DIM*jg+1] * renorm * ivnorm2[k];
           */
