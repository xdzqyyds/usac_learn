...
/* quantized value */
	...
	if(rd_idx[2*j]<ncd_2[0]) {
	    for(i=0;i<dim_2[0];i++) {
		qv_rd[lpc_order*j+i] = (float)((p_factor*out_p[i]
	...
	if(rd_idx[2*j+1]<ncd_2[1]) {
	    for(i=0;i<dim_2[1];i++) {
		qv_rd[lpc_order*j+dim_2[0]+i] = (float)((p_factor*out_p[dim_2[0]+i]
	...
	float tbl[], long dim, float wt[], float p_factor)
{
	...
	for(j=0;j<dim;j++) {
		c[j] = (float)(in[j]-(p_factor*out_p[j]+(1.-p_factor)*out_v[j]));
	}
